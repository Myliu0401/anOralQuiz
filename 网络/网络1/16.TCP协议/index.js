           ...
           TCP收发数据
               ...
               可靠传输
                 ...
                 发送每一小段都会带个头部，头部中就包含信息：
                  ...
                  http协议一次完整请求后就断开，为了释放内存，不然往后有其他请求的话，开辟的内存会越来越多。

                建立链接（三次握手）
                   链接的本质就是双方各自开辟一块内存空间，内存空间中主要是数据缓冲区和一些变量
                   服务器会监听一个端口

                   链接建立的过程需要经过三次数据报传输，因此称为三次握手

                  1. 客户端向服务端发送 SYN包（吐音：信）  （表示想要建立链接，此时客户端进入 SYN_SENT（吐音：信（普语），新（潮语））状态,也就是已发送状态）
                  2. 服务端收到SYN包后 发回 SYN包+ACK包 给客户端 （服务端表示同意，你那边呢。随后服务端进入 SYN_SENT状态，也就是已发送状态）
                ...
                销毁链接通道（四次挥手）
                  ...
                  第一次挥手
                  客户端向服务端发送 FIN（土音：菲）包 （表示客户端请求关闭链接。随后客户端进入FIN1状态，此时链接通道中客户端不能再向服务端发送数据，但能读取数据）
                 
                  第二次挥手
                  服务端收到 FIN包后，向客户端发送 ACK包 （服务端进入close状态，此后服务端不能再读取数据，但能发送数据）
                  客户端收到 ACK包后（进入FIN2状态，等待服务端发回FIN包）
                     这个阶段服务端仍然可以向客户端发送数据
                  
                  第三次挥手
                  服务端再向客户端发回 FIN包 （服务端进入LAST（辣死）状态, 此时服务器不能再发送数据。等待客户端发回ACK包）
            ...
            http和tcp的关系：
               ...
               每个http请求和响应，都要在tcp链接通道进行，请求和响应时独立的无法保持状态。因为链接通道销毁，无法判断后续请求的状态。




            TCP 是全双工通信（两端都可以发送和接收）。
            当一方（客户端）说“我不发数据了（FIN）”，只是表示它的发送方想关闭，但接收方向还开着。
            所以另一方（服务端）还可能有数据没发完，需要等它也主动说“我也不发了（FIN）”。
            因此需要两个 FIN，两次 ACK，合计四次。
