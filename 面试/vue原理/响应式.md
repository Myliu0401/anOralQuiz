...
由于上面的 4 个阶段的原因：所以执行`$set、$delete`进行重渲染，是调用第一个参数的记录的依赖，所以不怎么好，以为怕记录的依赖过多。
...
计算属性的原理：
vue 会遍历计算属性，然后执行 compuWatcher 函数为每个计算属性创建一个 watch。
...
initWatch 函数会先判断 属性值是否为对象如果是，则取出对象中的 handler 函数，该函数会作为 watch 实例要执行的函数。如果 handler 是字符串，则直接从当前的 vue 实例中取出该属性值作为 watch 实例要执行的函数。
...
function createWatch(vm, name, handler) {
  let options = handler === 'object' ? handler : {};

  if (handler === "object") {
    // 判断是否为对象类型
    options = handler; // 为监听器的配置对象
    handler = handler.handler; // 取出handler对象
  }
...
function Watcher(vm, name, cb, options){
   ...
   this.getter = parsePath(name);  // 会得到一个函数该函数会根据命名规则返回最低层属性
